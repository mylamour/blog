...
function isTaxonomyNode(value) {
  return typeof value === 'string' && value.trim().length > 0;
}

function flatHierarchy(value) {
  if (!Array.isArray(value) || value.length === 0) return null;
  const nodes = [];
  for (let index = 0; index < value.length; index += 1) {
    const node = value[index];
    if (!isTaxonomyNode(node)) return null;
    nodes.push(node);
  }
  return nodes;
}

function interpretTaxonomy(field, value) {
  if (isTaxonomyNode(value)) {
    return { valid: true, hierarchies: [[value]] };
  }
  if (!Array.isArray(value) || value.length === 0) {
    return { valid: false, hierarchies: [] };
  }

  if (field === 'tags') {
    const tags = flatHierarchy(value);
    return tags
      ? { valid: true, hierarchies: tags.map((tag) => [tag]) }
      : { valid: false, hierarchies: [] };
  }

  let hasExplicitHierarchy = false;
  for (let index = 0; index < value.length; index += 1) {
    if (Array.isArray(value[index])) {
      hasExplicitHierarchy = true;
      break;
    }
  }
  if (!hasExplicitHierarchy) {
    const hierarchy = flatHierarchy(value);
    return hierarchy
      ? { valid: true, hierarchies: [hierarchy] }
      : { valid: false, hierarchies: [] };
  }
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (Array.isArray(item)) {
      const hierarchy = flatHierarchy(item);
      if (!hierarchy) return { valid: false, hierarchies: [] };
      hierarchies.push(hierarchy);
    } else if (isTaxonomyNode(item)) {
      hierarchies.push([item]);
    } else {
      return { valid: false, hierarchies: [] };
    }
  }
  return { valid: true, hierarchies };
}
...
function validatePostSchema(posts) {
  ...
  for (const post of sortedPosts(posts)) {
    ...
    const validCategories = hasOwn(data, 'categories')
      && interpretTaxonomy('categories', data.categories).valid;
    const validTags = hasOwn(data, 'tags')
      && interpretTaxonomy('tags', data.tags).valid;
...
function validateTaxonomyCollisions(inventory) {
  ...
  for (const post of englishPosts) {
    for (const field of ['categories', 'tags']) {
      ...
      const taxonomy = interpretTaxonomy(field, post.data[field]);
      if (!taxonomy.valid) continue;
      for (const hierarchy of taxonomy.hierarchies) {
        const slugPath = [];
        const rawPath = [];
        for (const node of hierarchy) {
          slugPath.push(slugize(node, { transform: 1 }));
          rawPath.push(node);
          const routeKey = slugPath.join('/');
          const groupKey = `${field}\0${routeKey}`;
          if (!groups.has(groupKey)) {
            groups.set(groupKey, { field, routeKey, values: new Map() });
          }
          const group = groups.get(groupKey);
          const identity = JSON.stringify(rawPath);
          if (!group.values.has(identity)) {
            group.values.set(identity, {
              display: rawPath.length === 1 ? JSON.stringify(rawPath[0]) : identity,
              files: new Set()
            });
          }
          group.values.get(identity).files.add(post.file);
        }
      }
    }
  }
  ...
  for (const collision of collisions) {
    const values = [...collision.values.entries()]
      .map(([identity, entry]) => ({
        ...
        files: [...entry.files].sort(compareText)
      }))
    ...
    const details = values
      .map((entry) => `${entry.display} in ${entry.files.map((file) => JSON.stringify(file)).join(', ')}`)
      .join('; ');
    diagnostics.push(diagnosticFor(
      firstPost,
      collision.field,
