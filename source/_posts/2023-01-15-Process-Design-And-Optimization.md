...
以办公网安全治理举例(其中终端安全的细节可以参考这篇[浅谈终端安全](/End-User-Computer-Control-And-DLP/)), 终端安全这里就有黑白名单软件禁用的流程，软件统一推送的流程，Admin权限回收的流程，回收后的提权流程，证书推送的流程，数据泄漏后的处置流程，IT的资产管理流程，职工身份的转变流程等等。这里面就有IT和安全部门的协同，还有不同部门的员工办公需求等。那怎么在员工的行为规范中体现这些策略，怎么在实施中包含具体的控制点等就是设计时着重关注的。谁提，谁批，分支到哪，谁实施，哪里记录，哪里审计，哪里实施等。这些就是具体的工单覆盖，把流程统统的变成平台/系统中的Ticket。
